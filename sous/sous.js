			...
			function recipeRef(d){
				recipeNum += 1;
				this.num = recipeNum;
				this.name = d.name;
				this.link = set_default(d.link, "");
				this.T = set_default(d.T, "");
				this.CookTime = set_default(d.cookTime, "");
				this.comments = set_default(d.comments, "");
			}

			function create_addRecipe_button(){
				var txt = '<input type="button" id="addRecipe_but" value="Add">';
				$("#recipes").append(txt);
				$("#recipes").click(function(){
					//open menu to add recipes
			...
			function load_myNotes(){
				$.ajax({
					url: "./myNotes.json",
					dataType: "json",
					cache: false,
					success: function( data ) {
						$("#myNotes").val(data.value);
